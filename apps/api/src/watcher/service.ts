...
export class WatcherService implements OnModuleInit, OnModuleDestroy {
    ...
    this.watcher
      ...
      .on('ready', () => {
        ...
        this.cache
          .reconcileAll()
          .catch((error) =>
            this.logger.error(`Startup reconcile sweep failed: ${(error as Error).message}`),
          );
      });
  }
  ...
  private handleUpsert(event: 'add' | 'change', relativePath: string): void {
    ...
    this.cache
      ...
      .catch((error) =>
        this.logger.error(`Failed to reconcile ${relativePath}: ${(error as Error).message}`),
      );
  }
  ...
  private handleUnlink(relativePath: string): void {
    ...
    this.cache
      ...
      .catch((error) =>
        this.logger.error(`Failed to remove ${relativePath}: ${(error as Error).message}`),
      );
  }
