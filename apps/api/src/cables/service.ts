...
export class CablesService {
  async findBySetup(setupId: string): Promise<CableDto[]> {
    ...
    const portIds = deviceIds.length
      ? (await portsRepo.find({ where: { deviceId: In(deviceIds) } })).map((p) => p.id)
      : [];
  ...
  private async assertValid(
    ...
    const result = validateConnection({
      ...
      adapter: adapter
        ? {
            inputType: adapter.inputType as PortType,
            outputType: adapter.outputType as PortType,
            invertsPolarity: adapter.invertsPolarity,
          }
        : undefined,
      sourcePower: {
