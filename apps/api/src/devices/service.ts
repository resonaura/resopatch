...
export class DevicesService {
  ...
  async getPowerBudget(deviceId: string): Promise<PowerBudgetResult> {
    ...
    const targetPorts = targetPortIds.length
      ? await portsRepo.find({ where: { id: In(targetPortIds) } })
      : [];
    const targetPortById = new Map(targetPorts.map((p) => [p.id, p]));
    const targetDeviceIds = [...new Set(targetPorts.map((p) => p.deviceId))];
    const targetDevices = targetDeviceIds.length
      ? await devicesRepo.find({ where: { id: In(targetDeviceIds) } })
      : [];
    ...
    const maxOutputPowerW =
      device.powerMaxOutputPowerW ?? (portsCapacityW > 0 ? portsCapacityW : null);
