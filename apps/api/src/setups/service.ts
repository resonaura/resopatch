...
export class SetupsService {
  ...
  async create(dto: CreateSetupDto): Promise<SetupDto> {
    const setup = setupsRepo.create({
      name: dto.name,
      description: dto.description ?? null,
      checklistState: dto.checklistState ?? null,
    });
  ...
  async getGraph(setupId: string) {
    ...
    const ports = deviceIds.length
      ? await portsRepo.find({ where: { deviceId: In(deviceIds) } })
      : [];
    const portsByDevice = new Map<string, typeof ports>();
    for (const p of ports)
      portsByDevice.set(p.deviceId, [...(portsByDevice.get(p.deviceId) ?? []), p]);
    const furniture = deviceIds.length
      ? await furnitureRepo.find({ where: { deviceId: In(deviceIds) } })
      : [];
    const furnitureByDevice = new Map(furniture.map((f) => [f.deviceId, f]));

    const portIds = ports.map((p) => p.id);
    const cables = portIds.length
      ? await cablesRepo.find({ where: { sourcePortId: In(portIds) } })
      : [];
    const adapterIds = [
      ...new Set(cables.map((c) => c.adapterId).filter((id): id is string => Boolean(id))),
    ];
    const adapters = adapterIds.length
      ? await adaptersRepo.find({ where: { id: In(adapterIds) } })
      : [];
    ...
    return {
      devices: devices.map((d) => ({
        ...
        furniture: furnitureByDevice.has(d.id)
          ? toFurnitureDto(furnitureByDevice.get(d.id)!)
          : null,
      })),
      cables: cables.map((c) => ({
        ...toCableDto(c),
        adapterName: (c.adapterId && adapterById.get(c.adapterId)?.name) ?? null,
      })),
  ...
  async getInputList(setupId: string, hasKeys = true): Promise<InputListRow[]> {
    const devices = (await devicesRepo.find({ where: { setupId } })).filter(
      (d) => hasKeys || !d.attrs?.isKeysOnly,
    );
    ...
    return cables
      ...
      .filter(
        (
          ...
          sourcePort: NonNullable<typeof row.sourcePort>;
          targetPort: NonNullable<typeof row.targetPort>;
        } => {
  ...
  async getRider(setupId: string, hasKeys = true): Promise<RiderRow[]> {
    ...
    const cableGroups = new Map<
      string,
      { quantity: number; isUserOwned: boolean; length: number; cableType: string }
    >();
