...
import { deviceTypeKey, getDisplayName } from '../../lib/devices/naming';
import { inventoryStatusLabel } from '../../lib/i18n/enum-labels';
            ...
            <Modal.Body className="flex flex-col gap-3">
              <TextField isRequired autoFocus>
                ...
                <Input
                  ...
                  placeholder="e.g. Boss RC-5 Loop Station"
                />
              </TextField>
                  ...
                  <ListBox>
                    {Object.values(DeviceType).map((devType) => (
                      <ListBox.Item
                        key={devType}
                        id={devType}
                        textValue={t(deviceTypeKey(devType))}
                      >
                        <DeviceTypeIcon type={devType} className="text-default-500 h-3.5 w-3.5" />
              ...
              <Select
                ...
                onChange={(v) => setInventoryStatus(v as InventoryStatus)}
              >
                  ...
                  <ListBox>
                    {Object.values(InventoryStatus).map((status) => (
                      <ListBox.Item
                        key={status}
                        id={status}
                        textValue={inventoryStatusLabel(status, t)}
                      >
                  ...
                  <ListBox>
                    ...
                    {parentCandidates.map((d) => (
                      <ListBox.Item key={d.id} id={d.id} textValue={getDisplayName(d, t, language)}>
                        <DeviceTypeIcon type={d.type} className="text-default-500 h-3.5 w-3.5" />
