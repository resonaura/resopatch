...
import {
  ...
  formatConnectorPair,
  shortConnectorLabel,
} from '../../lib/cables/label';
import { maxLabelWidthForRun, MIN_LABEL_RUN_PX } from '../../lib/cables/label-clearance';
import { findLabelPoint } from '../../lib/routing/geometry/label-position';
import { roundedPathFromPoints, sampleAlongPath } from '../../lib/routing/geometry/rounded-path';
...
import { useCanvasRouting } from './state';
...
// in `routing/pathfinding/edge-routing.ts`) so a cap essentially never lands mid-bend.
const CAP_LENGTH = 32;
...
function findLongestPathSegment(
  ...
  const consider = (
    from: number,
    to: number,
    best: { x: number; y: number; deg: number; len: number } | null,
  ) => {
    let next = best;
...
function attachPathToLiveHandles(
  ...
  if (
    Math.hypot(s0.x - start.x, s0.y - start.y) < 2 &&
    Math.hypot(s1.x - end.x, s1.y - end.y) < 2
  ) {
    return routed;
  }
  ...
  const stubLen = Math.max(20, Math.min(48, Math.round(Math.abs(routed[1].x - routed[0].x) || 28)));
...
export default function RoutedEdge({
  ...
  const routedPoints = useCanvasRouting((state) => state.routes.get(id));
  const hasPlayedEntryAnimation = useCanvasRouting((state) => state.animatedRouteIds.has(id));
  const markRouteAnimated = useCanvasRouting((state) => state.markRouteAnimated);
  const hasRoute = Boolean(routedPoints && routedPoints.length >= 2);

  useEffect(() => {
    if (!hasRoute || hasPlayedEntryAnimation) return;
    const timer = window.setTimeout(() => markRouteAnimated(id), 180);
    return () => window.clearTimeout(timer);
  }, [hasRoute, hasPlayedEntryAnimation, id, markRouteAnimated]);
  ...
  const points = useMemo(
    ...
    [routedPoints, sourceX, sourceY, targetX, targetY, sourceHandleId, targetHandleId],
  );

  // Never reveal React Flow's temporary direct line while the routing worker is still computing.
  // Existing routed points remain mounted until PatchCanvas atomically publishes a replacement.
  if (!points || points.length < 2) return null;
  ...
  const path = roundedPathFromPoints(points, 16);
  ...
  return (
    <>
      <BaseEdge
        ...
        markerEnd={markerEnd}
        interactionWidth={14}
        className={hasPlayedEntryAnimation ? undefined : 'resopatch-route-enter'}
      />
      {underlayPath && (
        <path
          d={underlayPath}
      ...
      {powerConverter ? (
        <EdgeLabelRenderer>
          <div
            ...
            className="z-30 flex select-none items-center gap-2 rounded-md border border-neutral-700 bg-neutral-900/95 px-2.5 py-1 font-mono text-[11px] text-white shadow-2xl backdrop-blur-md"
          >
            <div className="flex h-4 w-4 shrink-0 items-center justify-center rounded bg-amber-500/20 text-[10px] font-bold text-amber-400">
              🔌
            </div>
            <div className="flex flex-col">
              <span className="text-[9px] font-semibold uppercase leading-none tracking-wider text-neutral-400">
