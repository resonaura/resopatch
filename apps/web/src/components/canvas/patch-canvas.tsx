...
import { PARALLEL_CABLE_GAP } from '../../lib/cables/label-clearance';
import {
  isPsuObstacleId,
  toRoutingEdges,
  toRoutingNodesWithNippleWalls,
} from '../../lib/routing/adapters/react-flow-routing';
import {
  applyCableManagement,
  type EdgePortMeta,
} from '../../lib/routing/post-processing/cable-management';
import { findLabelPoint } from '../../lib/routing/geometry/label-position';
import type { Point } from '../../lib/routing/types';
import {
  pickBestNipplePath,
  resnapRouteToMeasuredHandles,
} from '../../lib/routing/pathfinding/nipple-routing';
import {
  enforceOrthogonal,
  nudgeParallelRuns,
} from '../../lib/routing/post-processing/nudge-parallel';
import { pathHitsNodeBodies, type NodeBox } from '../../lib/routing/geometry/path-avoid-nodes';
import {
  basePortId,
  pickNearestSourceHandle,
  pickNearestTargetHandle,
} from '../../lib/routing/geometry/port-handles';
import { logAllRoutes } from '../../lib/routing/worker/debug-log';
import { hasCompleteRouteSnapshot } from '../../lib/routing/worker/snapshot';
import { buildSimpleOrthoPath } from '../../lib/routing/geometry/simple-ortho';
import { svgPathToPoints } from '../../lib/routing/geometry/svg-path-to-points';
import {
  edgeIdFromPsuCard,
  PSU_CARD_H,
  PSU_CARD_W,
...
} from './power-adapter-node';
import { patchCanvasEdgeTypes, patchCanvasNodeTypes } from './types';
import { CanvasRoutingProvider, useCanvasRouting } from './state';
...
function PatchCanvasInner({
  ...
  const publishRouting = useCanvasRouting((state) => state.publish);
  const psuNodes = useCanvasRouting((state) => state.powerAdapterNodes);
  ...
  useEffect(() => {
    // The worker store is global and is replaced asynchronously. Keep the last complete set on
    // screen until every edge in the current canvas has a route; on first load this leaves the
    // canvas empty instead of exposing React Flow's temporary straight lines.
    if (!hasCompleteRouteSnapshot(edges, avoidRoutes)) return;
    ...
    for (const e of edges) {
      ...
      const sNode = nodeLookup.get(e.source);
      const tNode = nodeLookup.get(e.target);
      if (!sNode || !tNode) {
        next.set(e.id, wasmPts);
        continue;
      }

      const best = pickBestNipplePath(wasmPts, sNode, tNode, e.sourceHandle, e.targetHandle, boxes);
    ...
    if (next.size !== edges.length) return;
    ...
    for (const [id, pts] of packed) {
      ...
      const sNode = e ? nodeLookup.get(e.source) : null;
      const tNode = e ? nodeLookup.get(e.target) : null;
    ...
    for (const e of edges) {
      ...
      for (const d of boxes) {
        const overlapX = box.x < d.x + d.width + 8 && box.x + box.width + 8 > d.x;
        const overlapY = box.y < d.y + d.height + 8 && box.y + box.height + 8 > d.y;
      ...
      for (const [id, pts] of finalRoutes) {
        ...
        const consider = (cand: Point[] | null) => {
          ...
          if (
            pathHitsNodeBodies(
              cand,
              boxes.filter((b) => b.id !== m.sourceId && b.id !== m.targetId),
              4,
            )
          )
            return;
        ...
        for (const psu of foreignPsu) {
          const ring = [psu.y - 28, psu.y + psu.height + 28, psu.x - 28, psu.x + psu.width + 28];
          // Horizontal rings
    ...
    publishRouting(finalRoutes, nextPsu);

    // Debug dump — open DevTools console, copy FULL_JSON or call __resopatchCopyRouteDump().
    logAllRoutes({
      nodes: [
        ...boxes.map((b) => {
          const n = nodeLookup.get(b.id);
          const device = n?.data?.device;
          const deviceName =
            device && typeof device === 'object' && 'name' in device ? device.name : null;
          const label =
  ...
  }, [avoidRoutes, edges, storeApi, setEdges, measureKey, handleEpoch, publishRouting]);
  ...
  const layoutKey = useMemo(() => {
    ...
    const edgePart = initialEdges
      .map(
        (e) =>
          `${e.id}:${e.source}:${e.target}:${basePortId(e.sourceHandle)}:${basePortId(e.targetHandle)}`,
      )
      .sort()
      .join('|');
  ...
  const psuNodeIds = useMemo(() => new Set(psuNodes.map((node) => node.id)), [psuNodes]);
    ...
    () =>
      edges.map((e) => ({
        ...
        data: {
          ...
          psuAsNode: psuNodeIds.has(psuCardIdForEdge(e.id)),
        },
      })),
    [edges, psuNodeIds],
  );
  ...
  const onNodesChange = useCallback(
    (changes: NodeChange<Node>[]) => {
      ...
      // The library reads its node ref synchronously, before React applies fresh dimensions.
      // A follow-up refresh is required for a newly mounted canvas (notably the pedalboard modal)
      // so the first route uses measured cards instead of stopping at the placeholder geometry.
      if (changes.some((change) => change.type === 'dimensions')) {
        requestAnimationFrame(refreshRouting);
      }
    },
    [onNodesChangeBase, refreshRouting, updateRoutingOnNodesChange],
  );
