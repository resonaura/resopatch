...
import { containerInternalGraph } from '../../lib/graph/container';
...
import type { DeviceNodeData } from '../devices/node';
import Inspector, { type Selection } from '../setup/inspector';
import PatchCanvas from '../canvas/patch-canvas';
...
export default function ContainerInsideModal({
  ...
  const positionedChildDevices = useMemo(() => {
    const power = childDevices.filter(
      (d) =>
        d.type === DeviceType.POWER_SUPPLY ||
        d.type === DeviceType.POWER_SPLITTER ||
        d.type === DeviceType.POWER_STRIP,
    );
    const unsortedSignal = childDevices.filter(
      (d) =>
        d.type !== DeviceType.POWER_SUPPLY &&
        d.type !== DeviceType.POWER_SPLITTER &&
        d.type !== DeviceType.POWER_STRIP,
  ...
  const { boundaryNodes, boundaryCables } = useMemo(() => {
    ...
    for (const cable of allCables) {
      ...
      if (sourceInChild !== targetInChild) {
        ...
        const extPort = sourceInChild
          ? allPortById.get(cable.targetPortId)
          : allPortById.get(cable.sourcePortId);
  ...
  const displayedDevices = useMemo(
    () => [...positionedChildDevices, ...boundaryNodes],
    [positionedChildDevices, boundaryNodes],
  );
  const displayedCables = useMemo(
    () => [...internalCables, ...boundaryCables],
    [internalCables, boundaryCables],
  );
  ...
  return (
    <Modal>
      <Modal.Backdrop isOpen onOpenChange={(open) => !open && onClose()}>
        <Modal.Container className="mx-auto h-[95vh] max-h-[95vh] !w-[98vw] w-[98vw] !max-w-[98vw] max-w-[98vw] overflow-hidden">
          <Modal.Dialog className="bg-surface border-default-200 flex h-full !w-full w-full !max-w-none flex-col overflow-hidden rounded-xl border p-0 shadow-2xl">
            <Modal.CloseTrigger />
            <div className="border-default-200 bg-surface flex items-center justify-between border-b px-5 py-3">
              <div className="flex items-center gap-3">
                <div className="bg-accent/15 text-accent flex aspect-square h-10 min-h-10 w-10 min-w-10 shrink-0 items-center justify-center rounded-lg">
                  <Layers className="aspect-square h-5 w-5 shrink-0" />
                </div>
                <div>
                  <h2 className="text-foreground text-base font-semibold">
                    {getDisplayName(containerDevice, t, language)} — {t('containerModal.title')}
                  </h2>
                  <p className="text-default-500 text-xs">
                    {t('containerModal.subtitle')
                      .replace('{devices}', String(childDevices.length))
                      .replace('{ports}', String(boundaryNodes.length))}
                  </p>
                </div>
              </div>
            ...
            <div className="bg-background flex min-h-0 flex-1 overflow-hidden">
              <div className="relative min-h-0 flex-1 overflow-hidden p-0">
                {childDevices.length === 0 ? (
                  <div className="text-default-500 flex h-full flex-col items-center justify-center gap-3">
              ...
              <div className="border-default-200 bg-surface h-full w-[320px] flex-none overflow-hidden border-l">
                <Inspector
