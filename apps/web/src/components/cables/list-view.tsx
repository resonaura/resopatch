...
import { cableTypeLabel } from '../../lib/cables/type-label';
...
export default function CableListView({
  devices,
  cables,
}: {
  devices: GraphDevice[];
  ...
  const zoneOf = (cable: GraphCable) =>
    portToDevice.get(cable.sourcePortId)?.ownerRole?.trim() || noZone;
  ...
  return (
    <div className="bg-background flex h-full min-h-0 flex-col overflow-y-auto p-4">
      <div className="mb-4 flex flex-wrap items-center gap-4">
        <div className="border-default-200 bg-surface-secondary/80 flex items-center rounded-lg border p-0.5">
          {(
            [
        ...
        <div className="flex flex-wrap items-center gap-2">
          <span className="text-default-500 text-[10px] font-medium uppercase tracking-wide">
            {t('cables.filterType')}
          </span>
          {allTypes.map((type) => {
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {allZones.map((z) => {
              ...
              return (
                <label
                  key={z}
                  className={`border-default-200 flex cursor-pointer items-center gap-1.5 rounded-full border px-2 py-1 text-[11px] transition-opacity ${
      ...
      <div className="border-default-200 bg-surface overflow-hidden rounded-xl border shadow-sm">
        <div className="border-default-200 bg-surface-secondary/50 flex items-center justify-between border-b px-4 py-2.5">
          <span className="text-foreground text-sm font-semibold">
            {t('cables.count', { count: visibleCables.length })}
          </span>
        </div>
        <div className="divide-default-100 divide-y">
          {visibleCables.length === 0 && (
            <div className="text-default-500 p-4 text-sm">{t('cables.empty')}</div>
          )}
          {visibleCables.map((c) => {
            ...
            return (
              <div key={c.id} className="flex items-center gap-3 p-3">
                ...
                />
                <CableIcon className={`text-default-400 shrink-0 ${FALLBACK_ICON_CLASS}`} />
                <div className="min-w-0 flex-1">
                  <div className="text-foreground truncate text-sm font-medium">
                    {c.productName ?? cableTypeLabel(c.cableType, t)} — {c.length}
                    {t('meter')}
                    {c.color ? ` (${c.color})` : ''}
                    {!c.isUserOwned && (
                      <span className="text-default-500 ml-2 text-[10px]">
                        {t('cables.venueProvided')}
                      </span>
                    )}
                  </div>
                  <div className="text-default-500 truncate text-xs">
                    {sourceDevice?.name ?? '?'} → {targetDevice?.name ?? '?'} ·{' '}
                    {zoneOf(c) === noZone ? noZone : formatOwnerRole(zoneOf(c), t)}
                  </div>
                </div>
              </div>
            );
