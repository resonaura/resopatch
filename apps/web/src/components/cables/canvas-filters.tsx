...
export default function CableCanvasFilters({
  ...
  const filtersActive = category !== 'all' || hiddenConnectors.size > 0 || hiddenZones.size > 0;
  ...
  return (
    <div className="border-default-200 bg-surface/95 pointer-events-auto flex max-w-[min(920px,calc(100vw-8rem))] flex-wrap items-center gap-2 rounded-xl border px-2.5 py-1.5 shadow-lg backdrop-blur-md">
      ...
      {presentConnectors.length > 0 && (
        <div className="flex flex-wrap items-center gap-1.5">
          <span className="text-default-500 text-[9px] font-medium uppercase tracking-wide">
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          {allZones.map((z) => {
            ...
            return (
              <label
                key={z}
                className={`border-default-200 flex cursor-pointer items-center gap-1 rounded-full border px-1.5 py-0.5 text-[10px] transition-opacity ${
      ...
      {onReset && filtersActive && (
        <button
          type="button"
          onClick={onReset}
          className="border-default-200 text-default-500 hover:bg-surface-secondary hover:text-foreground rounded-md border px-2 py-0.5 text-[10px] font-medium transition-colors"
        >
