...
import { cableTypeLabel, portTypeLabel } from '../../lib/i18n/enum-labels';
import { useI18n } from '../../lib/i18n';
import { formatI18nText } from '../../lib/i18n/text';

function guessCableType(source: PortDto, target: PortDto): CableType {
  if (POWER_PORT_TYPES.includes(source.portType) && POWER_PORT_TYPES.includes(target.portType))
    return CableType.POWER_LINE;
  if (source.portType === PortType.MIDI_DIN && target.portType === PortType.MIDI_DIN)
    return CableType.MIDI;
  if (source.portType === PortType.WIRELESS && target.portType === PortType.WIRELESS)
    return CableType.CONTROL_LINK;
  const usbTypes: PortType[] = [PortType.USB_A, PortType.USB_B, PortType.USB_C];
  if (usbTypes.includes(source.portType) && usbTypes.includes(target.portType))
    return CableType.USB_DATA;
...
export default function NewCableModal({
  ...
  const [cableType, setCableType] = useState<CableType>(() =>
    guessCableType(sourcePort, targetPort),
  );
  ...
  const create = useMutation({
    ...
    onError: (err) =>
      toast(err instanceof Error ? err.message : t('newCableModal.createError'), {
        variant: 'danger',
      }),
  });
          ...
          <Modal.Dialog>
            ...
            <Modal.Body className="flex flex-col gap-3">
              <div className="border-default-200 bg-surface-secondary rounded-lg border p-2.5 text-xs">
                <div>
                  {sourceDeviceName} — {formatI18nText(sourcePort.name, language)} (
                  {portTypeLabel(sourcePort.portType, t)})
                </div>
                <div className="text-default-500">→</div>
                <div>
                  {targetDeviceName} — {formatI18nText(targetPort.name, language)} (
                  {portTypeLabel(targetPort.portType, t)})
                </div>
              </div>
                ...
                <Select.Popover>
                  <ListBox>
                    ...
                    {(adapters.data ?? []).map((a) => (
                      ...
                      >
                        {formatI18nText(a.name, language)} ({portTypeLabel(a.inputType, t)} →{' '}
                        {portTypeLabel(a.outputType, t)})
                      </ListBox.Item>
                    ))}
                  </ListBox>
                </Select.Popover>
              ...
              <TextField>
                ...
                <Input
                  ...
                  placeholder={t('newCableModal.colorPlaceholder')}
                />
              </TextField>
            </Modal.Body>
            <Modal.Footer>
