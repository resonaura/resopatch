...
function DeviceThumb({
  ...
  if (device.imageUrl) {
    const isStorage =
      !device.imageUrl.startsWith('data:') && !/^https?:\/\//i.test(device.imageUrl);
    const src = isStorage ? `/img/${device.imageUrl}?w=128` : device.imageUrl;
    return (
      <img
        src={src}
        alt=""
        className={`aspect-square shrink-0 rounded bg-black/20 object-contain p-0.5 ${className}`}
      />
    );
  }
  return (
    <DeviceTypeIcon
      type={device.type}
      className={`text-default-500 aspect-square shrink-0 ${FALLBACK_ICON_CLASS}`}
    />
  );
}
...
export default function Sidebar({
  ...
  return (
    <div className="border-default-200 bg-surface flex h-full min-h-0 flex-col border-r">
      <div className="flex items-center justify-between gap-2 p-3">
        <h2 className="text-default-500 text-xs font-semibold uppercase tracking-wide">
          {t('sidebar.inventory')}
        </h2>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto pb-3">
        {GROUPS.map((group) => {
          const items = filtered.filter(
            (d) => d.inventoryStatus === group.status && !d.parentDeviceId,
          );
          ...
          return (
            ...
            >
              <Disclosure.Heading>
                <Disclosure.Trigger className="text-default-500 w-full px-2 py-1.5 text-left text-[11px] font-medium uppercase tracking-wide">
                ...
                <div className="flex flex-col gap-0.5">
                  {items.map((d) => (
                    <button
                      ...
                      title={d.type}
                      className={`hover:bg-surface-secondary flex items-center gap-2 rounded-md border-l-2 px-2 py-1.5 text-left text-[12.5px] transition-colors ${
                        selectedId === d.id
                          ? 'border-l-accent bg-surface-secondary'
                ...
                {filteredCables.map((c) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      />
                      <CableIcon className="text-default-500 h-3.5 w-3.5 shrink-0" />
                      <span className="truncate">
                        {c.productName ?? c.cableType} — {c.length}
                        {t('meter')}
                        {owner ? ` · ${owner}` : ''}
                      </span>
                    </button>
