...
import { formatI18nText, mergeI18nText } from '../../lib/i18n/text';
import CheckboxField from '../forms/checkbox-field';
import ImagePicker from '../images/picker';
import RiderSpecSheet from './rider-spec-sheet';
...
function Section({
  ...
  return (
    <Disclosure
      ...
      className="border-default-200 border-t pt-2"
    >
      <Disclosure.Heading>
        <Disclosure.Trigger className="text-default-500 flex w-full items-center gap-1.5 text-left text-xs font-semibold">
...
function DeviceForm({
  ...
  const save = useMutation({
    ...
    onError: (err) =>
      toast(err instanceof Error ? err.message : t('inspector.saveError'), { variant: 'danger' }),
  });
  ...
  const updatePort = useMutation({
    mutationFn: (vars: { id: string; dto: Parameters<typeof api.updatePort>[1] }) =>
      api.updatePort(vars.id, vars.dto),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['graph', setupId] }),
  });
  ...
  const powerBudget = useQuery({
    ...
    enabled:
      isPowerSourceDevice &&
      (device.ports.some(
        (p) => p.power.maxOutputPowerW != null || p.power.maxOutputCurrentMA != null,
      ) ||
    ...
    <div className="flex flex-col gap-3">
      ...
      <TextField>
        ...
        <div className="flex items-center gap-2">
          <Input
            value={colorAttr}
            onChange={(e) =>
              setForm((f) => ({ ...f, attrs: { ...f.attrs, color: e.target.value } }))
            }
          ...
          {colorAttr && (
            <span
              className="border-default-300 h-6 w-6 shrink-0 rounded-full border"
              style={{ backgroundColor: colorAttr }}
              title={colorAttr}
            />
          )}
        </div>
      ...
      {showPowerSection && (
        <Section title={t('inspector.powerSection')} icon={Zap}>
          {!isPowerSourceDevice && (
            <CheckboxField
              isSelected={form.powerRequired}
              onChange={(v) => commitField('powerRequired', v)}
            >
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setForm((f) => ({
                        ...
                        power: {
                          ...f.power,
                          voltageV: e.target.value === '' ? undefined : Number(e.target.value),
                        },
                      }))
                    }
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setForm((f) => ({
                        ...
                        power: {
                          ...f.power,
                          currentMA: e.target.value === '' ? undefined : Number(e.target.value),
                        },
                      }))
                    }
                    onBlur={() => commitField('power', form.power)}
          ...
          {isPowerSourceDevice && (
            <>
              <p className="text-default-500 text-[11px] font-medium">
                {t('inspector.powerSourceCapacitySection')}
              </p>
              {enumSelect(
                Object.values(PowerSourceType),
                form.powerSourceType,
              ...
              {powerBudget.data && (
                ...
                >
                  <div>
                    ...
                    {powerBudget.data.maxOutputPowerW != null
                      ? ` / ${powerBudget.data.maxOutputPowerW}W`
                      : ''}
      ...
      <Section
        title={t('inspector.portsSection').replace('{count}', String(device.ports.length))}
        icon={CableIcon}
      >
        {device.ports.map((port) => (
          <div key={port.id} className="grid grid-cols-[1fr_1fr_70px_auto] items-center gap-1.5">
            ...
            <select
              ...
              className="select__trigger border-default-200 bg-surface-secondary h-9 rounded-lg border px-2 text-xs"
              onChange={(e) =>
                updatePort.mutate({ id: port.id, dto: { portType: e.target.value as PortType } })
              }
            >
            ...
            <select
              ...
              onChange={(e) =>
                updatePort.mutate({
                  id: port.id,
                  dto: { direction: e.target.value as PortDirection },
                })
              }
            >
            ...
            <Button
              ...
              size="sm"
              variant="danger"
              onPress={() => deletePort.mutate(port.id)}
            >
      ...
      {children.length > 0 && (
        <Section
          title={t('inspector.kitSection').replace('{count}', String(children.length))}
          icon={Package}
        >
          {children.map((child) => (
            <button
              ...
              className="border-default-200 hover:bg-surface-secondary flex items-center gap-2 rounded-md border px-2 py-1.5 text-left text-xs"
            >
              <DeviceTypeIcon type={child.type} className="text-default-500 h-3.5 w-3.5 shrink-0" />
              <span className="truncate">{getDisplayName(child, t, language)}</span>
            </button>
          ))}
          <p className="text-default-500 text-[11px]">{t('inspector.kitNotice')}</p>
        </Section>
      )}
      ...
      <Section title={t('inspector.notesSection')} icon={StickyNote}>
        ...
        {attrsError && <p className="text-danger text-sm">{attrsError}</p>}
      </Section>
...
function CableForm({
  ...
  const [productNameEdit, setProductNameEdit] = useState(() =>
    formatI18nText(cable.productName, language),
  );

  return (
    <div className="flex flex-col gap-3">
      <div className="border-default-200 bg-surface-secondary rounded-lg border p-2.5 text-xs">
        <div>
          {sourceDevice ? getDisplayName(sourceDevice, t, language) : ''} —{' '}
          {formatI18nText(sourcePort?.name, language)}
        </div>
        <div className="text-default-500">↓</div>
        <div>
          {targetDevice ? getDisplayName(targetDevice, t, language) : ''} —{' '}
          {formatI18nText(targetPort?.name, language)}
        </div>
      </div>
      {enumSelect(
      ...
      <TextField>
        ...
        <Input
          type="number"
          step="0.1"
          defaultValue={cable.length}
          onBlur={(e) => save.mutate({ length: Number(e.target.value) })}
        />
      </TextField>
      <TextField>
        <Label>{t('inspector.color')}</Label>
        <Input
          defaultValue={cable.color ?? ''}
          onBlur={(e) => save.mutate({ color: e.target.value || undefined })}
        />
      </TextField>
      ...
      <CheckboxField
        isSelected={cable.isPatchCable}
        onChange={(v) => save.mutate({ isPatchCable: v })}
      >
        {t('inspector.patchCable')}
      </CheckboxField>
      <CheckboxField
        isSelected={cable.isUserOwned}
        onChange={(v) => save.mutate({ isUserOwned: v })}
      >
...
export default function Inspector({
  ...
  if (!selection) {
    return (
      <div className="border-default-200 bg-surface text-default-500 h-full min-h-0 overflow-y-auto border-l p-4 text-sm">
  ...
  if (selection.kind === 'device') {
    ...
    return (
      <div className="border-default-200 bg-surface h-full min-h-0 overflow-y-auto border-l p-3.5">
