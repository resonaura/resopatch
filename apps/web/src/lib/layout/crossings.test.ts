...
describe('segmentsIntersect', () => {
  it('detects a simple X cross', () => {
    expect(segmentsIntersect({ x: 0, y: 1 }, { x: 2, y: 1 }, { x: 1, y: 0 }, { x: 1, y: 2 })).toBe(
      true,
    );
  });
  ...
  it('returns false for parallel horizontal segments', () => {
    expect(segmentsIntersect({ x: 0, y: 0 }, { x: 2, y: 0 }, { x: 0, y: 1 }, { x: 2, y: 1 })).toBe(
      false,
    );
  });

  it('returns false for non-crossing T-shape', () => {
    expect(segmentsIntersect({ x: 0, y: 0 }, { x: 2, y: 0 }, { x: 3, y: 0 }, { x: 3, y: 2 })).toBe(
      false,
    );
  });
...
describe('resolveNodeOverlaps', () => {
  it('separates two identical-position cards', () => {
    ...
    const gap = 40;
    const overlapX = Math.min(a.x + 200 + gap, b.x + 200 + gap) - Math.max(a.x, b.x);
    const overlapY = Math.min(a.y + 100 + gap, b.y + 100 + gap) - Math.max(a.y, b.y);
