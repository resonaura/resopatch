...
import { findFreeSlot, greedySwapMinimize, resolveNodeOverlaps } from './crossings';
import {
  deviceNameIncludes,
  deviceNameLower,
  FAMILY_ORDER,
...
import type { LayoutCable, LayoutDevice, LayoutResult, LogicalFamily, ZoneName } from './types';
import { normalizeOwnerZone, zoneOf } from './zones';
...
function layoutZone(
  ...
  const powerRankOf = ranksForDevices(
    zoneDevices.filter(isPowerInfrastructure),
  ...
  while (i < familyBlocks.length) {
    ...
    const pair = next && canPair(cur.fam, next.fam) && cur.width + COLUMN_GAP + next.width < 2200;
...
function layoutContainerChildren(
  ...
  const power = children.filter(isPowerInfrastructure);
  const signal = children.filter((d) => !isPowerInfrastructure(d));
...
export function computeAutoLayout(
  ...
  const sizeOf = (id: string): { width: number; height: number } => {
    ...
    const expandedH =
      (hasImage ? IMAGE_BANNER_H : 0) + CARD_CHROME_H + portsBlock + expandCtrl + insideBtn + 8;
  ...
  for (const amp of mainDevices.filter((d) => d.type === DeviceType.AMPLIFIER)) {
    const mic = mainDevices.find(
      (d) =>
        ...
        zoneOf(d) === zoneOf(amp) &&
        (deviceNameIncludes(d, 'e835') ||
          deviceNameIncludes(d, 'combo') ||
          deviceNameIncludes(d, 'cab mic') ||
          deviceNameIncludes(d, 'amp mic') ||
          (deviceNameIncludes(d, 'Sennheiser') && normalizeOwnerZone(d.ownerRole) === 'vox')),
    );
    if (mic) {
      ...
      consumer = mainDevices.find(
        (d) =>
          ...
          (n.includes('tu-3') || n.includes('tu3')
            ? deviceNameIncludes(d, 'TU-3') || deviceNameIncludes(d, 'tuner')
            : n.includes('cinders')
              ? deviceNameIncludes(d, 'Cinders')
              : false),
      );
    }
  ...
  for (const amp of mainDevices.filter((d) => d.type === DeviceType.AMPLIFIER)) {
    const outlet = mainDevices.find(
      (d) =>
        zoneOf(d) === zoneOf(amp) &&
        deviceNameIncludes(d, 'venue outlet') &&
        (deviceNameIncludes(d, 'combo') || deviceNameLower(d).includes('combo')),
    );
    if (outlet) {
  ...
  for (const anker of mainDevices.filter(
    (d) => deviceNameIncludes(d, 'Anker') && d.type === DeviceType.POWER_STRIP,
  )) {
  ...
  const stageBox = mainDevices.find(
    (d) =>
      d.type === DeviceType.STAGE_BOX ||
      deviceNameIncludes(d, 'stage box') ||
      deviceNameIncludes(d, 'стейдж'),
  );
  const motu = mainDevices.find(
    (d) => deviceNameIncludes(d, 'MOTU') || deviceNameIncludes(d, 'UltraLite'),
  );
  if (stageBox && motu) {
