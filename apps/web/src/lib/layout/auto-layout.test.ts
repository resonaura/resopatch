...
import { computeAutoLayout, computeFlowRanks } from './auto-layout';
import { logicalFamily } from './device-classification';
...
describe('logicalFamily', () => {
  it('splits vocal vs guitar gear', () => {
    expect(
      logicalFamily(
        dev({ id: 'sm58', type: DeviceType.MICROPHONE, name: 'Shure SM58 Vocal Microphone' }),
      ),
    ).toBe('vocal');
    expect(
  ...
  it('groups laptop, keys, and stage IO', () => {
    expect(
      logicalFamily(dev({ id: 'mb', type: DeviceType.LAPTOP, name: 'MacBook Pro Playback' })),
    ).toBe('laptop');
    expect(
      logicalFamily(dev({ id: 'kl', type: DeviceType.KEYBOARD, name: 'Arturia KeyLab' })),
    ).toBe('keys');
    expect(
      logicalFamily(dev({ id: 'motu', type: DeviceType.AUDIO_INTERFACE, name: 'MOTU UltraLite' })),
    ).toBe('io');
    expect(
      logicalFamily(dev({ id: 'sb', type: DeviceType.STAGE_BOX, name: 'Venue Stage Box' })),
    ).toBe('io');
  });
});
...
describe('computeAutoLayout zones', () => {
  it('places different owner roles in separated horizontal regions', () => {
    ...
    const cables = [{ sourcePortId: 'a1p', targetPortId: 'a2p' }];
