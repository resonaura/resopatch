...
import { segmentHitsBox, type NodeBox } from './path-avoid-nodes';
import type { CardBox, Point, Side } from './port-handles';
import { exteriorStubX, segmentTunnelsCard } from './port-handles';
...
export function buildSimpleOrthoPath(
  ...
  if (sourceSide === targetSide) {
    const outerX = sourceSide === 'left' ? Math.min(stubS.x, stubT.x) : Math.max(stubS.x, stubT.x);
    cands.push(
  ...
  if (Math.abs(midX - stubS.x) > 16 && Math.abs(midX - stubT.x) > 16) {
    cands.push(simplify([s, stubS, { x: midX, y: stubS.y }, { x: midX, y: stubT.y }, stubT, e]));
  }
  ...
  if (sourceBox && targetBox) {
    ...
    for (const cy of [topY, botY]) {
      cands.push(simplify([s, stubS, { x: stubS.x, y: cy }, { x: stubT.x, y: cy }, stubT, e]));
    }
  }
