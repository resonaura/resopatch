...
export function sourceSideFromHandleId(
...
export function targetSideFromHandleId(
  handleId: string | null | undefined,
  fallbackPath?: Point[],
): Side {
...
export function exteriorStubX(box: CardBox, side: Side, stubLen: number): number {
  return side === 'right' ? Math.round(box.x + box.width + stubLen) : Math.round(box.x - stubLen);
}
...
export function snapPathToNipples(
  ...
  if (mid.length === 0) {
    ...
    const startI = Math.abs(first.x - stubS.x) > 0.5 || Math.abs(first.y - stubS.y) > 0.5 ? 1 : 0;
