...
import { defaultSourceHandle, defaultTargetHandle } from '../geometry/port-handles';
 ...
 */
export function cablePlugGenderForPort(
  portType: string | undefined | null,
): CablePlugGender | null {
...
export function graphCableToEdge(
  ...
  if (isPowerAdapter) {
    const targetVoltage =
      tPort?.power.voltageV ??
      sPort?.power.voltageV ??
      tDev?.power.voltageV ??
      sDev?.power.voltageV ??
      9;
    const targetCurrent =
  ...
  return {
    ...
    // Unique dual-nipple ids (see the device node port row and geometry/port-handles.ts).
