...
import { STUB_LANE_GAP } from '../../cables/label-clearance';
import { enforceOrthogonal, type Point } from './nudge-parallel';
import { pathHitsNodeBodies, type NodeBox } from '../geometry/path-avoid-nodes';
import type { Side } from '../geometry/port-handles';
...
export function rebuildWithStubs(
  ...
  const stubSX =
    sourceSide === 'right' ? Math.round(padS.x + stubDepthS) : Math.round(padS.x - stubDepthS);
  const stubTX =
    targetSide === 'right' ? Math.round(padT.x + stubDepthT) : Math.round(padT.x - stubDepthT);
...
function assignLanes(items: { edgeId: string; y: number }[]): Map<string, number> {
  const sorted = [...items].sort((a, b) => a.y - b.y || a.edgeId.localeCompare(b.edgeId));
