...
import { closerFace, pickBestNipplePath, preferredSides } from './nipple-routing';
import type { NodeBox } from '../geometry/path-avoid-nodes';
...
describe('pickBestNipplePath', () => {
  it('ALWAYS picks nearer L/R nipples for a clear free-air hop (not the far side)', () => {
    ...
    expect(best!.sourceHandle).toBe('p1-src-right');
    expect(best!.targetHandle).toBe('p2-tgt-left');
