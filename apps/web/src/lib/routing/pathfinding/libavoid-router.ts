...
// obstacle-router 0.1.2 exposes incompatible copies of its internal router interfaces.
// Derive the public method parameter types here and keep the compatibility cast localized.
type ShapePinHost = Parameters<typeof ShapeConnectionPin.createForShape>[0];
type ConnectionPinHost = Parameters<typeof ConnEnd.fromShapePin>[0];
...
function targetDirFlag(dir?: 'left' | 'right'): number {
  // Match the legacy edge router: only explicit 'right' faces right; unset → left.
...
function polygonToPoints(route: {
  size(): number;
...
export function routeWithLibavoid(
  ...
  for (const o of obstacles) {
    ...
    const shape = new ShapeRef(
      router as unknown as ConstructorParameters<typeof ShapeRef>[0],
      new Rectangle(tl, br),
      nextObjectId++,
    );
  ...
  for (const spec of edges) {
    ...
    if (src) {
      ...
      const pin = ShapeConnectionPin.createForShape(
        src.shape as unknown as ShapePinHost,
        pinClass,
        xOff,
        yOff,
        true,
      ...
      srcEnd = ConnEnd.fromShapePin(src.shape as unknown as ConnectionPinHost, pinClass);
    } else {
      srcEnd = ConnEnd.fromPoint(
        new AvoidPoint(spec.start.x, spec.start.y),
        sourceDirFlag(spec.sourceDir),
      );
    }
    ...
    if (tgt) {
      ...
      const pin = ShapeConnectionPin.createForShape(
        tgt.shape as unknown as ShapePinHost,
        pinClass,
        xOff,
        yOff,
        true,
      ...
      pin.setExclusive(true);
      tgtEnd = ConnEnd.fromShapePin(tgt.shape as unknown as ConnectionPinHost, pinClass);
    } else {
      tgtEnd = ConnEnd.fromPoint(
        new AvoidPoint(spec.end.x, spec.end.y),
        targetDirFlag(spec.targetDir),
      );
    }
