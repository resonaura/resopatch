 ...
 *
 * **Important:** this module must never import the libavoid adapter.
 * That package is ~0.5MB of sync JS and used to freeze the tab on every import of
 * Canvas components stay on the main thread; libavoid runs only in `worker/route-worker.ts`.
 *
 * This file owns path search and route orchestration. Reusable geometry and post-processing live
 * in their dedicated sibling modules.
 */

import { PARALLEL_CABLE_GAP } from '../../cables/label-clearance';
import {
  insetOwnObstacle as ownBodyRect,
  pathsViolateDrc,
  pathSelfIntersects,
  segmentCrossesRect,
  segmentsCross,
} from '../geometry/intersections';
import { findLabelPoint } from '../geometry/label-position';
import { dropMicroSegments, reduceBends, simplifyColinear } from '../geometry/path-simplification';
import { resolveOverlaps } from '../post-processing/resolve-overlaps';
import type { EdgeRouteSpec, Point, RectObstacle } from '../types';
export type { EdgeRouteSpec, Point, RectObstacle } from '../types';
...
function astar(
  ...
  while (heap.size > 0 && steps++ < MAX_EXPANSIONS) {
    ...
    for (let d = 0; d < DIRS.length; d++) {
      ...
      if (ncx < bounds.minCx || ncx > bounds.maxCx || ncy < bounds.minCy || ncy > bounds.maxCy)
        continue;
...
function trySimpleRoute(
  ...
  if (sSign === tSign) {
    const loopX =
      sSign > 0 ? Math.max(stubS.x, stubT.x) + stubLen : Math.min(stubS.x, stubT.x) - stubLen;
...
export function makeSegmentClear(spec: EdgeRouteSpec, obstacles: RectObstacle[]) {
  const otherObstacles = obstacles.filter(
    (o) => o.id !== spec.sourceNodeId && o.id !== spec.targetNodeId,
  );
...
export function findPath(spec: EdgeRouteSpec, obstacles: RectObstacle[]): Point[] {
  ...
  if (found) {
    ...
    const sourceStubX =
      sSign === 1
        ? Math.max(start.x + sStub, firstCorner.x)
        : Math.min(start.x - sStub, firstCorner.x);
    const sourcePoints: Point[] = [
      start,
      { x: sourceStubX, y: start.y },
      { x: sourceStubX, y: firstCorner.y },
    ];

    const targetStubX =
      tSign === 1 ? Math.max(end.x + tStub, lastCorner.x) : Math.min(end.x - tStub, lastCorner.x);
    const targetPoints: Point[] = [
      { x: targetStubX, y: lastCorner.y },
...
function bundleFanInRoutes(
  ...
  const applyGroup = (group: EdgeRouteSpec[]) => {
    const unique = [...new Map(group.map((g) => [g.id, g])).values()].filter(
      (g) => !processed.has(g.id),
    );
    ...
    const firstCorridorX =
      gapHi > gapLo ? gapLo + Math.max(0, gapHi - gapLo - packWidth) / 2 : (spanLo + spanHi) / 2;
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (!placed) {
        ...
        const order =
          Math.abs(hyBelow - midY) <= Math.abs(hyAbove - midY)
            ? [hyBelow, hyAbove]
            : [hyAbove, hyBelow];
...
export function findBestPath(
  candidates: EdgeRouteSpec[],
  obstacles: RectObstacle[],
): { path: Point[]; spec: EdgeRouteSpec } {
...
/** Adds a verified-clear detour when coincident straight routes would render as one line. */
...
export function finalizeRoutes(
  ...
  if (adapterObstacles.length > 0) {
    ...
    for (const spec of edges) {
      ...
      const path = hitsAdapter ? dropMicroSegments(simplifyColinear(findPath(spec, live2))) : prev;
  ...
  // Both worker engines may return shared collinear runs. Separate them before cosmetic bends so
  // every consumer (main canvas and pedalboard modal alike) receives the same DRC-safe result.
  // A safety rollback can reveal a smaller overlap group after the first pass. A second bounded
  // pass separates that remainder without introducing an unbounded convergence loop.
  for (let pass = 0; pass < 2; pass++) {
    routes = resolveOverlaps(routes, obstacles, edges);
  }
  ...
  for (const [id, pts] of routes) {
    const otherSegments = Array.from(segmentsByEdge, ([otherId, segs]) =>
      otherId === id ? [] : segs,
    ).flat();
    ...
    if (ok) {
      ...
      for (const o of obstacles) {
        ...
        if (o.id.startsWith('trace:') || o.id.startsWith('adapter-card-')) continue;
        if (
          bent.slice(0, -1).some((p, i) => segmentCrossesRect(p, bent[i + 1], o, OBSTACLE_PADDING))
        ) {
          ok = false;
          break;
        }
 ...
 */
export function computeRoutesLegacy(
  obstacles: RectObstacle[],
  edges: EdgeRouteSpec[],
): Map<string, Point[]> {
