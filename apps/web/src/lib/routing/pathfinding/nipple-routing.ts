...
import { pathHitsNodeBodies, simplifyClearPath, type NodeBox } from '../geometry/path-avoid-nodes';
import {
  basePortId,
  snapPathToNipples,
  sourceHandleOptions,
  ...
  type Point,
  type Side,
} from '../geometry/port-handles';
import { buildSimpleOrthoPath } from '../geometry/simple-ortho';
...
interface RoutingNode {
  ...
  internals: {
    ...
    handleBounds?: Partial<
      Record<
        'source' | 'target',
        { id?: string | null; x: number; y: number; width: number; height: number }[] | null
      >
    >;
  };
}
...
function listHandles(internalNode: RoutingNode, kind: 'source' | 'target'): HandleHit[] {
  const list = internalNode.internals.handleBounds?.[kind] ?? [];
  const abs = internalNode.internals.positionAbsolute ?? internalNode.position ?? { x: 0, y: 0 };
  const out: HandleHit[] = [];
  for (const h of list) {
    if (!h.id) continue;
...
export function pickBestNipplePath(
  ...
  for (const s of srcOpts) {
    for (const t of tgtOpts) {
      ...
      const score =
        nippleAirLength(s, t) * 2 + pathLength(path) + Math.max(0, path.length - 2) * 40;
  ...
  wasmCombos.sort(
    (a, b) =>
      Number(b.nearest) - Number(a.nearest) ||
      nippleAirLength(a.s, a.t) - nippleAirLength(b.s, b.t),
  );
