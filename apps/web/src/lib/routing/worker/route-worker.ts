...
import { computeRoutesLegacy, finalizeRoutes } from '../pathfinding/edge-routing';
import { routeWithLibavoid } from '../pathfinding/libavoid-router';
...
self.onmessage = (event: MessageEvent<RouteWorkerRequest>) => {
  ...
  try {
    ...
    console.log(
      `[routeWorker] id=${id} engine=${engine} edges=${edges.length} ms=${Date.now() - t0}`,
    );
  } catch (err) {
