...
import { buildPortNodeMap, containerInternalGraph, splitMainCanvasGraph } from './container';

let portSeq = 0;

function dev(
  id: string,
  overrides: Partial<GraphDevice> & { ports?: PortDto[] } = {},
): GraphDevice {
  return {
    id,
    setupId: 'setup',
...
function port(
  deviceId: string,
  name: string,
  direction: PortDirection = PortDirection.BI,
): PortDto {
  return {
    id: `${deviceId}-${name}-${portSeq++}`,
    deviceId,
    name,
...
describe('splitMainCanvasGraph', () => {
  ...
  it('handles a container with both internal and external cabling at once (the pedalboard shape)', () => {
    ...
    const boundary = result.boundaryPortsByContainer
      .get('board')
      ?.map((p) => p.id)
      .sort();
