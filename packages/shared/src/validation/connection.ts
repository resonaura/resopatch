...
import { PowerProfile } from '../domain/types.js';
...
export function validateConnection(input: ConnectionInput): ConnectionValidationResult {
  ...
  if (sourceIsPower || targetIsPower) {
    if (!sourceIsPower || !targetIsPower) {
      return {
        valid: false,
        error: 'Power connectors cannot mix with signal connectors.',
        warnings,
      };
    }
  ...
  if (adapter) {
    if (!familiesOverlap(sourcePortType, adapter.inputType)) {
      return {
        valid: false,
        error: 'Adapter input does not match the source connector.',
        warnings,
      };
    }
    if (!familiesOverlap(adapter.outputType, targetPortType)) {
      return {
        valid: false,
        error: 'Adapter output does not match the target connector.',
        warnings,
      };
    }
  ...
  for (const portType of sideToCheck) {
    if (!allowedTypes.includes(portType)) {
      return {
        valid: false,
        error: `${cableType} cable cannot terminate on a ${portType} port.`,
        warnings,
      };
    }
  }
  ...
  if (sourceIsPower && targetIsPower && sourcePower && targetPower) {
    ...
    if (
      ...
      warnings.push(
        `Voltage mismatch: source provides ${sourcePower.voltageV}V, target expects ${targetPower.voltageV}V.`,
      );
    }
  }
